...
import type { EditorFeatureConfig, HeadingLevel } from "../config";
...
interface SlashItem {
  ...
  icon: LucideIcon;
  group: SlashGroup;
  // Feature flag / heading level this item needs. Absent means always
  // available (StarterKit block). Flag-gated items must not be offered when
  // the flag is off: the extension isn't loaded, so the command would throw.
  feature?: keyof EditorFeatureConfig;
  level?: HeadingLevel;
...
export const SlashCommand = Extension.create({
  ...
  addOptions() {
    return {
      ...
      // Wired from the resolved editor config in extensions.ts. null means
      // "no gating" so a bare SlashCommand still offers everything.
      features: null as EditorFeatureConfig | null,
      headingLevels: null as HeadingLevel[] | null,
    };
  },

  addProseMirrorPlugins() {
    // Computed once per editor — items() runs on every keystroke and must
    // return a stable reference for the unfiltered case (a fresh array there
    // resets SlashList's keyboard-nav index on each press).
    const { features, headingLevels } = this.options;
    const availableItems = SORTED_SLASH_ITEMS.filter((item) => {
      if (item.feature && features && !features[item.feature]) return false;
      if (item.level && headingLevels && !headingLevels.includes(item.level)) return false;
      return true;
    });
    return [
        ...
        render: () => {
          ...
          return {
            onStart: (props) => {
              ...
              // Body portal — outside the editor wrapper, so the theme
              // (color scheme + inline token overrides) is copied on.
              inheritEditorTheme(renderer.element as HTMLElement, props.editor.view.dom);
              document.body.appendChild(renderer.element);
              position(props);
              // Dismiss on outside click — the plugin only exits on its own
              // when the caret moves or the query breaks. Uses the default
              // suggestion plugin key (none is configured above).
            ...
            onKeyDown: (props) => {
              if (props.event.key === "Escape") {
                // Full plugin exit, not just hiding the element — otherwise
                // the suggestion stays active invisibly and a follow-up
                // Enter would still run the hidden selected item.
