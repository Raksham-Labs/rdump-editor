...
import { serializeBlockHTML } from "../../core/blockHtml";
import { serializeImageMarkdown, type ImageAttrs } from "../images/ResizableImage";
 ...
 * nothing is lost. An explicitly aligned textblock also disqualifies: the
 * pipe spelling has nowhere to put the alignment, so it would silently snap
 * back to left on the next reload.
 */
function isCellSerializable(cell: PMNode): boolean {
  ...
  if (!only) return true;
  if (only.isTextblock) {
    const align = only.attrs.textAlign;
    return !align || align === "left";
  }
  return only.type.name === "image";
}
