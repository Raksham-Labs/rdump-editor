...
import { BlockCaption } from "../../ui/BlockCaption";
import { openMediaLightbox } from "../../ui/MediaLightbox";
import { useEditorColorScheme } from "../../ui/useEditorColorScheme";
...
function MermaidView({ node, updateAttributes, editor }: NodeViewProps) {
  ...
  const scheme = useEditorColorScheme(rootRef);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          // Mermaid bakes colors into the SVG, so CSS tokens can't theme it —
          // pick its own theme per render from the editor's color scheme
          // (the effect re-runs on theme flips via the `scheme` dep).
          mermaid.initialize({
            startOnLoad: false,
  ...
  return (
    <NodeViewWrapper ref={rootRef} className="rdump-mermaid">
